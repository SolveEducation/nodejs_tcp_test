    ...
    try{
        if( data_from_client.request === "register" ){
            ...
            if (match !== false){
                ...
                chat_list.waiting_users.splice(chat_list.waiting_users.indexOf(record.User_ID),1);
                chat_list.waiting_users.splice(chat_list.waiting_users.indexOf(socket_chatmate.user_detail.User_ID),1);
            ...
            if(typeof chat_list.active_chat[record.User_ID] !== 'undefined'){
                let receiver = clients[chat_list.active_chat[record.User_ID]];
                response.done = 1;
                response.status = "incoming chat";
                response.message = record.Chat_Name + "> " + data_from_client.message;
                receiver.write(JSON.stringify(response));
            }else{
                response.message = "There is no chatmate";
